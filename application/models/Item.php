...
class Item extends CI_Model
{
	...
	*/
	public function get_info($item_id)
	{
		...
		else
		{
			...
			log_message('debug',print_r($item_obj,TRUE));
			return $item_obj;
		}
	}
	...
	*/
	...
	public function get_search_suggestions($search, $filters = array('is_deleted' => FALSE, 'search_custom' => FALSE), $unique = FALSE, $limit = 25)
	{
		...
		$this->db->select($this->get_search_suggestion_format('item_id, name, pack_name, expire_date, '));
		$this->db->from('items');
	...
	public function get_hsn_code_suggestions($search)
	{
		$suggestions = [];
		$this->db->distinct();
		$this->db->select('hsn_code,id');
		$this->db->from('item_hsn_code');
		$this->db->like('hsn_code', $search);
		$this->db->where('deleted', 0);
		$this->db->order_by('hsn_code', 'asc');
		foreach($this->db->get()->result() as $row)
		{
			$suggestions[] = array('label' => $row->hsn_code);
		}

		return $suggestions;
	}

	public function hsn_code_tax_percentage($hsn_code)
		{	
			$this->db->select ('item_hsn_code.tax_percentage') ;
			$this->db->from ('item_hsn_code'); 
