...
class Split_items_new extends Secure_Controller
{
	public function __construct()
	{
		parent::__construct('Split_items_new');
	}

	public function index()
	{
		 $data['table_headers'] = $this->xss_clean(get_split_items_barcode_manage_table_headers());

		 $this->load->view('split_items/manage_barcode', $data);
	}
	...
	*/
	public function search()
	{
		...
		// $split_items = $this->Split_item->search_split($search, $filters, $limit, $offset, $sort, $order);
		// $total_rows = $this->Split_item->get_found_rows_split($search, $filters);
        $split_items = $this->Split_item_new->search_split($search, $filters, $limit, $offset, $sort, $order);
		$total_rows = $this->Split_item_new->get_found_rows($search, $filters);
		...
		{
			$data_rows[] = $this->xss_clean(get_split_item_barcode_data_row($split_items, $count));
			$count++;
		}
   ...
   //Get row
	...
	{
		$data_row = $this->xss_clean(get_split_item_barcode_data_row($this->Split_item_new->get_info($row_id)));
