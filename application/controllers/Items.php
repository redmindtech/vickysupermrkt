...
class Items extends Secure_Controller
{
	...
	public function suggest_hsn_code()
	{
		$suggestions = $this->xss_clean($this->Item->get_hsn_code_suggestions($this->input->get('term')));

		echo json_encode($suggestions);
	}


	// tax percentage hsn code based append get value
	public function hsn_code_tax($hsn_code)
    {

        $result = $this->Item->hsn_code_tax_percentage($hsn_code);
        $pending = 0;
        if ($result != null) {
            foreach ($result as $row) {
               
				
				$pending = $row->tax_percentage;
				$hsn_code_tax = $pending/2;
				// var_dumb($pending);
	...
	public function get_row($item_ids)
	{
		...
		foreach($item_infos->result() as $item_info)
		{
			...
			// var_dump($result[$item_info->item_id]);
			log_message('debug',print_r($result[$item_info->item_id],TRUE));
		}
	...
	public function view($item_id = NEW_ITEM)
	{
		...
		{
			$item_info->expire_date = date('Y-m-d H:i:s');
